...
from api.serializer import (
    ...
    APIConfigurationSerializer, InterviewAssistantSerializer,
    CreateAssistantSerializer, PhoneNumberSerializer,
    RegisterPhoneNumberSerializer, InterviewCallSerializer,
    MakeCallSerializer
)
        ...
        try:
            ...
            try:
                ...
                if not config.is_vapi_configured:
                    return Response({'error': 'Vapi API key not configured'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            system_message = self.create_interview_system_message(knowledge_text, knowledge_urls)
            professional_first_message = self.create_professional_first_message(first_message, knowledge_text, knowledge_urls)
            ...
            try:
                ...
                return Response({'error': f"Invalid JSON response from Vapi API: {response.text}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            if not response.ok:
                ...
                return Response({'error': f"Vapi API error ({response.status_code}): {error_message}"}, status=response.status_code)
            ...
            return Response({'error': f"Vapi API error: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
...
- End with summary questions about their experience"""
    ...
    def create_professional_first_message(self, base_message, knowledge_text="", knowledge_urls=""):
        ...
        try:
            ...
            try:
                ...
                if not config.is_twilio_configured:
                    return Response({'error': 'Twilio credentials not configured'}, status=status.HTTP_400_BAD_REQUEST)
                twilio_client = Client(config.twilio_account_sid, config.twilio_auth_token)
            except APIConfiguration.DoesNotExist:
                return Response({'error': 'Twilio credentials not configured'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            try:
                ...
                if not config.is_vapi_configured or not config.is_twilio_configured:
                    return Response({'error': 'Both Vapi API key and Twilio credentials must be configured'}, status=status.HTTP_400_BAD_REQUEST)
                vapi_key = config.vapi_api_key
                twilio_client = Client(config.twilio_account_sid, config.twilio_auth_token)
            except APIConfiguration.DoesNotExist:
                return Response({'error': 'API configuration not found'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        try:
            ...
            try:
                config = APIConfiguration.objects.get(user=request.user)
                if not config.is_vapi_configured:
                    return Response({'error': 'Vapi API key not configured. Please set it in the API Configuration section.'}, status=status.HTTP_400_BAD_REQUEST)
                vapi_key = config.vapi_api_key
            except APIConfiguration.DoesNotExist:
                return Response({'error': 'Vapi API key not configured'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                phone_number = PhoneNumber.objects.get(
                    user=request.user, 
                    vapi_phone_number_id=twilio_phone_number_id
                )
            except (InterviewAssistant.DoesNotExist, PhoneNumber.DoesNotExist):
                return Response({'error': 'Assistant or phone number not found or does not belong to user'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if not response.ok:
                ...
                return Response({'error': f"Vapi call API error ({response.status_code}): {error_message}"}, status=response.status_code)
...
class CallDetailView(APIView):
    ...
    def get(self, request, call_id):
        try:
            ...
            try:
                call = InterviewCall.objects.get(vapi_call_id=call_id, user=request.user)
    ...
    def update_call_from_vapi_data(self, call, call_data):
        ...
        if call_data.get('startedAt'):
            try:
                call.started_at = datetime.fromisoformat(call_data['startedAt'].replace('Z', '+00:00'))
            except:
                pass
                
        if call_data.get('endedAt'):
            try:
                call.ended_at = datetime.fromisoformat(call_data['endedAt'].replace('Z', '+00:00'))
            except:
                pass
        ...
        if call_data.get('transcript'):
            ...
            if isinstance(call_data['transcript'], list):
                call.transcript_text = "\n".join([
                    ...
                    for item in call_data['transcript'] if isinstance(item, dict)
                ])
