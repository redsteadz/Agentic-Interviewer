...
class InterviewAssistant(models.Model):
    ...
    configuration = models.JSONField(default=dict, blank=True)  # Store full Vapi configuration
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class InterviewCall(models.Model):
    ...
    assistant = models.ForeignKey(InterviewAssistant, on_delete=models.CASCADE, related_name='calls')
    phone_number = models.ForeignKey(PhoneNumber, on_delete=models.CASCADE, related_name='calls')
    customer_number = models.CharField(max_length=20)
    
    # Call details
    status = models.CharField(max_length=20, choices=CALL_STATUS_CHOICES, default='queued')
